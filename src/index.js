...
// })
router.post('/get-msg', async ctx => {
    let rsdata = ctx.request.body
    if (!rsdata["msgPwd"]) {
        rsdata = JSON.parse(Object.keys(ctx.request.body)[0].replace(/\$/g, "."))
    }
    // console.log(rsdata)
    let { msgPwd: pwd } = rsdata
    let msgData = await apiModel.getMsg(pwd)
    ctx.body = msgData


})
router.post('/write-msg', async ctx => {
    let rsdata = ctx.request.body
    if (!rsdata["msgPwd"]) {
        rsdata = JSON.parse(Object.keys(ctx.request.body)[0].replace(/\$/g, "."))
    }
    let { msgPwd: pwd, msg } = rsdata
    let writeMsgCallbackData = await apiModel.writeMsg(pwd, msg)
    ctx.body = writeMsgCallbackData
})
