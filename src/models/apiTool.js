...
module.exports = {
    getMsg(pwd) {
        return new Promise((resolve, reject) => {
            db.query("select * from `message` where pwd = ? limit 15", [pwd], function (err, rs) {
                if (err) {
                    reject({ "msg": "服务器错误" });
                } else {
                    resolve({ "msg": "success", "msgData": rs })
                }
            })
        })
    },
